...
import { mainData, contentEpisodes } from "./episodesNet";
let movies = mainData;
...
const videoPlayerSecondB = document.querySelector('.btns2');
...
function getDataFromUrl() {
    ...
    seariesTable = queryParams.get('series');

    specificMovie = movies.filter((element) => {
        return element.episodesTable == seariesTable;
    })

    specificEp = contentEpisodes.filter((elem) =>{
        return elem.EPid == movieId;
    })

    specificMovie = specificMovie[0];
    specificEp = specificEp[0]

    document.title = "Watch " + specificMovie.title + " " + specificEp.EPNo + " - Cinemateca";
    video_player.src = specificEp.EPUrl;
    movieTitle.innerText = specificEp.EPTitle;
    movieDesc.innerText = specificEp.EPDescri;
...
<h1 id="header_title">${specificEp.EPTitle}</h1>
<p id="header_pra">${specificEp.EPDescri}</p>
<div class="btns">
    ...
    bigPosterImg.src = `${specificEp.EPImg}`
    ...
    movieCategoris[2].innerHTML = `${specificEp.EPNo} <span></span>`;
    movieCategoris[3].innerHTML = `${specificEp.EPDate}`;
...
function moreLikeThis() {
    ...
    let moreLikeThis = movies.filter((e) => {
        return (((e.genre1 == gen1 || e.genre1 == gen2 || e.genre1 == gen3) ||
            ...
            (e.contentType === 'series' || e.contentType === 'tv shows') && (e.id != specificMovie.id));
    });
