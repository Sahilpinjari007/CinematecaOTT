...
import { mainData } from "./moviePageNet";
let movies = mainData;
...
const noInternetCard = Array.from(document.querySelectorAll('.no-Internect-cards'));
const noInternetSlider = document.getElementsByClassName('no-Internet-slider')[0];
let left_scroll = Array.from(document.querySelectorAll('[data-left-scroll]'));
let right_scroll = Array.from(document.querySelectorAll('[data-right-scroll]'));
let scrolling_section = Array.from(document.querySelectorAll('[data-scrolling-section]'));
...
const allScrollingSection = Array.from(document.querySelectorAll('.more-like-this-section'));
...
setTimeout(function () {
    ...
    noInternetCard[0].classList.add('active');
    noInternetCard[1].classList.add('active');
...
<h6>Duration: <span id="header_dur">${specificMovie.movieDuration}</span></h6>
...
<div class="btns">
    ...
    // video_player.src = `${specificMovie.url}`;
    watchBtnClick();

    movieTitle.innerText = specificMovie.title;
    ...
    movieDesc.innerText = specificMovie.description;
    movieDownloadBtn.href = specificMovie.downloadWebsiteUrl;

}

// Relate Topic Trailers section code
function relatedTopics() {
    ...
    let moreLikeThis = movies.filter((e) => {
        return (((e.genre1 == gen1 || e.genre1 == gen2 || e.genre1 == gen3) ||
            ...
            (e.contentType === 'trailer') && (e.id != movieId));
    });
    ...
    if (moreLikeThis.length <= 0) {
        ...
        scrolling_section[1].appendChild(p);
        scrolling_section[1].classList.add('sectionNoData');
        return;
    }
    else {
        scrolling_section[1].classList.remove('sectionNoData');
    }
    ...
    moreLikeThis.forEach(element => {
        ...
        const url = defUrl + `movies.html?${queryString}`;
        ...
        scrolling_section[0].appendChild(card);
    })
}
...
function moreLikeThis() {
    ...
    let moreLikeThis = movies.filter((e) => {
        return (((e.genre1 == gen1 || e.genre1 == gen2 || e.genre1 == gen3) ||
            ...
            (e.genre14 == gen1 || e.genre14 == gen2 || e.genre14 == gen3) ||
            (e.genre15 == gen1 || e.genre15 == gen2 || e.genre15 == gen3)) &&
            (e.contentType === 'movie') && (e.id != movieId));
    });
    ...
    `
        ...
        scrolling_section[1].appendChild(card);
    })
}
    ...
    for (let i = 0; i < navElemArr.length; i++) {
        ...
        navElemArr[i].addEventListener('click', () => {
            ...
            allScrollingSection.forEach((elem) => {
                elem.classList.toggle('moreLikeActive')
            })
...
// search bar searching code 
function searchBarSearching() {
...
// all scroller code start
{
    ...
    for (let i = 0; i < scrolling_section.length; i++) {
        ...
        left_scroll[i].addEventListener('click', () => {
            scrolling_section[i].scrollLeft -= 500;
        })
        right_scroll[i].addEventListener('click', () => {
            scrolling_section[i].scrollLeft += 500;
        })
    }
}
// all scroller code end